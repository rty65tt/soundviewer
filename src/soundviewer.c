...
#define HEIGHT 320	// height (odd number for centre line)
...
void DrawTimeLine(HDC dc, QWORD pos, DWORD col, DWORD txtcol, DWORD bgcol, DWORD y)
{
	...
	switch (m) {
		...
		case WM_PAINT:
			if (GetUpdateRect(h,0,0)) {
				...
				DrawTimeLine(dc,loop[0],0x00ffaa, 0x33cc88, 0x212121, HEIGHT/2 - 20); // loop start
				DrawTimeLine(dc,loop[1],0x0044ff, 0x3377ff, 0x212121, HEIGHT/2 + 5); // loop end
				DrawTimeLine(dc,BASS_ChannelGetPosition(chan,BASS_POS_BYTE),0xffffff, 0x212121, 0xdddddd,10); // current pos
				EndPaint(h,&p);
			}
			return 0;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,LPSTR lpCmdLine, int nCmdShow)
{
	...
	wc.lpszClassName = "Sound Viewer";
	if (!RegisterClass(&wc) || !CreateWindowA("Sound Viewer",
			"Sound Viewer (left-click to set loop start, right-click to set end)",
